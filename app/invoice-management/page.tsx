...
export default function InvoicePage() {
  ...
  const currentPageInvoices = invoices.slice(
      ...
      {showCreateInvoice ? (
        ...
        <div className="p-4 md:p-6">
          ...
          <header className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
            <div>
              <h1 className="text-[22px] font-semibold text-business_settings_black_text">
                Invoice List
              </h1>
          ...
          <div className="bg-[#fee0e0] rounded-md p-4 mb-6 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2">
            <div className="text-business_settings_black_text text-[14px]">
              You have{" "}
              <span className="font-semibold">{invoices.length}/50</span>{" "}
              invoices created.
            </div>

            <Link
              href="#"
              className="text-business_settings_black_text text-[14px] underline font-medium"
            >
                      ...
                      <tr className="border-b border-sidebar_gray_border">
                        <th className="py-3 px-4 text-left">
                          <div className="flex items-center">
                            ...
                            <span className="text-[14px] font-medium text-sidebar_black_text">
                              Invoice ID
                            </span>
                          </div>
                        </th>
                        ...
                        <th className="py-3 px-4 text-center text-[14px] font-medium text-sidebar_black_text">
                          Action
                        </th>
                    ...
                    <tbody>
                      {currentPageInvoices.map((invoice) => (
                        ...
                        >
                          ...
                          <td className="py-4 px-4 text-[14px] text-business_settings_black_text">
                            ₹
                            {invoice.totalPayableAmount.toLocaleString(
                              "en-IN",
                          ...
                          <td className="py-4 px-4">
                            <div className="relative">
                              <button
                                className={`inline-flex items-center px-3 py-1 rounded-full text-[13px] cursor-pointer ${getStatusColor(
                                  invoice.status
                                )}`}
                                onClick={() =>
                                  setShowStatusMenu(
                      ...
                      {currentPageInvoices.length === 0 && (
                        <tr>
                          <td
                            colSpan={6}
                            className="py-8 text-center text-sidebar_black_text"
                          >
                            No invoices found
                          </td>
                        </tr>
                  ...
                  >
                    <div className="flex justify-between items-center p-4 border-b border-sidebar_gray_border">
                      ...
                      <div className="relative">
                        <button
                          ...
                          onClick={() =>
                            setShowStatusMenu(
                              showStatusMenu === invoice.id ? null : invoice.id
                            )
                          }
                        >
                          {invoice.status}
                        ...
                        {showStatusMenu === invoice.id && (
                          <div className="absolute right-0 z-10 mt-2 w-48 rounded-md shadow-lg bg-white ring-1 ring-black ring-opacity-5">
                            <div className="py-1" role="menu">
                              {["PENDING", "PAID", "OVERDUE", "CANCELLED"].map(
                                (status) => (
                                  <button
                                    key={status}
                                    className={`block w-full text-left px-4 py-2 text-sm ${
                    ...
                    <div className="p-4">
                      <div className="grid grid-cols-2 gap-3 mb-4">
                        <div>
                          <div className="text-[12px] text-sidebar_black_text">
                            Invoice Date
                          </div>
              ...
              {invoices.length > 0 && (
                <div className="flex flex-col sm:flex-row justify-between items-center gap-4 mt-6">
                  ...
                  <div className="flex items-center gap-2 order-1 sm:order-2">
                    ...
                    {Array.from({
                      length: Math.min(Math.ceil(totalItems / itemsPerPage), 6),
                    }).map((_, index) => (
                      <button
                        key={index + 1}
                    ...
                    <button
                      onClick={() =>
                        activePage < Math.ceil(totalItems / itemsPerPage) &&
                      ...
                      <div className="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                        ...
                        <div className="mt-2">
                          <p className="text-sm text-gray-500">
                            ...
                            {selectedInvoices.length} selected invoices? This
                            action cannot be undone.
                          </p>
                        </div>
                      </div>
